...
Codex、Codex、Gemini CLI、Copilot CLI、OpenCode、Antigravity、Cursor、Kiro、CodeBuddy、OpenClaw、Trae。每个 agent 有独立的 skills 目录，以及各自的读取优先级链 —— 精确的 fallback 顺序见 README.md 的 "Supported Agents" 表（部分 agent 会以 `~/.agents/skills/` 或 `~/.Codex/skills/` 作为次级来源）。
